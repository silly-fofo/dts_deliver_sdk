...
public class DStoreRecordBuilder {
    ...
    public SchemafulProducerRecord build(String topicName, int partition, Schema schema, SchemaMetaData schemaMetaData, Record record, boolean needTimestamp) throws IOException {
        ...
        if (needTimestamp) {
            return new SchemafulProducerRecord(topicName, 0, System.currentTimeMillis()/*getTimestampMs(record)*/, schemafulRecord, schemaMetaData, data);
        } else {
            return new SchemafulProducerRecord(topicName, 0, schemafulRecord, schemaMetaData, data);
        }
    }
